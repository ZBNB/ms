...
import msg.domain.MsgConnect;
import msg.domain.MsgHead;
import msg.domain.MsgSubmit;
/**
 * 短信接口容器，单例获得链接对象
 *
 */
public class MsgContainer{
	...
	private static boolean sendShortMsg(String msg,String cusMsisdn){
		try {			
			...
			submit.setServiceId(MsgConfig.getServiceId()); //企业代码
			
			List<byte[]> dataList=new ArrayList<byte[]>();
			dataList.add(submit.toByteArry());
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发短短信，序列号为:"+seq);
	...
	private static boolean sendLongMsg(String msg,String cusMsisdn){
		try {
			...
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发长短信，序列号为:"+seqId);
